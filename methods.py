...
def mixup_data(x, y, alpha=1.0, use_cuda=True):
    ...
    batch_size = x.size()[0]
    if use_cuda:
        #index = jittor.misc.randperm(batch_size)#.cuda()
        index = torch.randperm(batch_size).cuda()
    else:
        #index = jittor.misc.randperm(batch_size)
...
class LabelAwareSmoothing(torch.nn.Module):
...
class LearnableWeightScaling(torch.nn.Module):
    def __init__(self, num_classes):
        super(LearnableWeightScaling, self).__init__()
        self.learned_norm = torch.nn.Parameter(torch.ones(1, num_classes))
